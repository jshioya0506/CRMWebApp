...
public class CustomersFacade {
	...
	public boolean doSearch (
		...
		// 検索条件1：失効日=0
		SelectQuery empCustomerQuery = new SelectQuery(Employee.class);
		Expression empCustomer01
			= ExpressionFactory.matchExp(
					NCCustomer.LOST_YMD_PROPERTY, Integer.valueOf(0));
		empCustomerQuery.setQualifier(empCustomer01);

		// 検索条件2：職員コード=%入力値% ※入力値が"*"だったら条件として指定しない
		/*
		if( !(staffCode.equals("*")) ) {
			Expression empCustomer02
			= ExpressionFactory.matchExp(
		...
		*/
		...
		if( !(companyCode.equals("*")) ) {
			Expression empCustomer03
			= ExpressionFactory.matchExp(
					NCCustomer.CUSTOMERCD_PK_COLUMN, companyCode);
			empCustomerQuery.andQualifier(empCustomer03);
		}
