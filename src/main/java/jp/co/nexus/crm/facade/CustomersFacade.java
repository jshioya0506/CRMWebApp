	...
	public boolean doSearch (
		...
		if( !(staffCode.equals("*")) )
		{
			...
			= exprExpire.andExp(ExpressionFactory.matchExp(
					NCCustomer.EMPLOYEE_PROPERTY, staffCode));
		}
		...
		// 検索条件[失効日が0 かつ 職員コード かつ 顧客コード]
		// 職員コード、顧客コードが"*"以外の場合のみ条件に追加
		customerQuery.setQualifier(exprExpire);
		...
		// 検索結果を顧客情報のモデルに設定
		List<NCCustomer> customersSearch = (List<NCCustomer>)context.performQuery(customerQuery);

		for (NCCustomer customer : customersSearch) {
			CustomerInfoBean infoBean = new CustomerInfoBean();
